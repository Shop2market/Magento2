...
class Product extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public const MODULE_NAME = 'Adcurve_Adcurve';
    ...
    {
        return false;
        $this->_logger->log(100, print_r($preparedData, true));
        if (!$preparedData) {
            return false;
        }
    ...
    private function _unsetIrrelevantAttributes()
    {
        ...
        /*
        ORGINAL
        if (count($this->exclude_these_attributes) > 0) {
    ...
    */
    public function getParentProductUrl(\Magento\Catalog\Model\Product $product, $storeId = null, $parentId)
    {
        ...
        return $this->urlHelper->getUrl($urlPath . $suffix, [ '_scope' => $storeId, '_nosid' => true ]);
    }
    ...
    */
    ...
    private function _addLogicComplexAttributes($product, $storeId, $productHasOrigData)
    {
        ...
        if ($product->getTypeId() == "simple") {
            ...
            if (
                (isset($newProductData['configurable_id']) && $newProductData['configurable_id'] == $newProductData['simple_id']) &&
                ...
                $this->productData['simple_id'] = $newProductData['simple_id'];
                ...
                $this->productData['deeplink'] = $newProductData['deeplink'];
                $this->_sendProductToUpdate($this->productData);
            } else {
    ...
    private function _addImageAttributes()
    {
        ...
        foreach ($imageKeys as $imageKey) {
            if (
                isset($this->productData[$imageKey])
    ...
    public function getProductCategoriesArray($shiftAmount = 2, $delimiter = ' > ')
    {
        ...
        foreach ($categoryCollection as $category) {
            ...
            if (
                isset($this->_fullCategoryPathsMappingArray[$category->getPath()])
    ...
    */
    ...
    public function addPriceAttributes($data, $product, $storeId, $cycleAllPriceAttributes = true)
    {
        ...
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $productPrice = $objectManager->get('Magento\Catalog\Model\Product\Type\Price');
        $finalPrice = $productPrice->calculatePrice(
            $data['price'],
            $data['special_price'],
            $data['special_from_date'],
            $data['special_to_date']
    ...
    public function getPriceInclAndExclTax(int $productId): array
    {
        ...
        return [ 'incl' => $priceExcludingTax, 'excl' => $priceExcludingTax ];
     ...
     * @param array $data
     */
    public function _sendProductToUpdate($preparedData)
    {
        if (!$preparedData) {
            return false;
        }
